...
df.rates = load.hausser2019.data() %>% 
  dplyr::rename_with(.fn=Pxx,px='hausser2019.rates',s='_',.cols=-gene)
...
df.cond_exp = load.lahtvee2017.data() %>%  dplyr::rename(orf=geneId) %>%
              rename_with(.fn=Pxx, px='lahtvee2017.exp',s='_',.cols=-orf)

exp_var = df.cond_exp %>% group_by(orf) %>% 
  summarise(var_mrna= var_(c_across(contains("mrna"))),
            var_prot= var_(c_across(contains("log10_prot"))),
            var_trans = var_(c_across(contains("log10_trans")))) %>%
