...
PROT.HL = load.belle2006.data() %>% 
          ...
          left_join( load.christiano2014.data(), by=c('ORF'='ensg'))
...
df.codons = bind_cols(orf=names(CDS), Biostrings::trinucleotideFrequency(CDS,step = 3), len.cds=lengths(CDS)) %>% dplyr::rename_with(.fn=Pxx,px="sgd")
...
df.HLprot = PROT.HL %>%
            dplyr::select(ORF,halflife.avg,halflife_mn_corrected,degradation_rates_min_1,t1_2_min) %>%
            dplyr::rename(villen2017.HL_prot=halflife.avg,
                          belle2006.HL_prot=halflife_mn_corrected,
                          christiano2014.HL_prot = t1_2_min,
                          christiano2014.deg_rate = degradation_rates_min_1)
