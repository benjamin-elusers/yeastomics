...
df.codons = bind_cols(orf=names(CDS), Biostrings::trinucleotideFrequency(CDS,step = 3), len_cds=lengths(CDS)) %>% dplyr::rename_with(.fn=Pxx,px="sgd")
 
### Expression in different conditions
df.cond_exp = load.lahtvee2017.data()%>%  dplyr::select(orf=geneId) %>%
