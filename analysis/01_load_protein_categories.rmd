...
suppressPackageStartupMessages({
  ...
  library(hablar)
  library(here)
  source(here("src","__setup_yeastomics__.r"))
})
...
Yeast is a model unicellular organism belonging to the fungi kingdom, within the eukaryotes ("true nucleus") domain of life. The yeast genome of 12Mbp seems tiny when compared to higher eukaryotes, which is at least one order of magnitude more compact in size. Nonetheless, the yeast proteome contains \~6000 protein-coding genes, making it only 3 to 4 times smaller than eukaryotes. Strictly quantitatively, the yeast genome/proteome would be comparable to the largest bacteria, similar to those of E. coli (4.5Mbp/4300) or B. subtilis (4.3Mbp/4800).

The yeast cellular volume is estimated around 50um\^3 for haploid cells which is occupied at 80% by the cellular proteome (42 Millions of expressed macromolecules). The cell interior is incredibly crowded with a protein density of around 3-4 millions of molecules per femtoliter. Despite the crowdedness, proteins are freely diffusing within the cell at an admirably fast pace of around 0.2us from one extremity to another.

Yet, the dynamic arrangements between millions of macromolecules within the cell is not trivial.

## Proteome evolution

The approach to understand the organization of the yeast proteome consist in searching for factors influencing the evolution of proteins.

As previously reported, protein abundance holds a particularly important role in driving molecular evolution of protein, explaining up to 50% of the variability in conservation rate among proteins.

The general trend is that highly expressed proteins are the most conserved following the chemical law of mass action. The association of protein expression with sequence conservation was found to be supported by several underlying mechanisms, yet their degree of complementarity is still a matter of active debate.

How abundance really affects protein evolution is likely to require multi-causal explanations which we will not expand here. (e.g. translation cost, protein/mRNA stability and degradation, misinteractions ...)

Nonetheless, protein abundance cannot fully account for the conservation of all residues in proteins. Even worse, the effect of abundance is likely to be involved in many biological processes, creating a potentially strong confounding factor that may bias most evolutionary comparison between proteins.

Here, we explore a number of quantitative and qualitative variables (referred as features and properties) derived from proteins, in order to unveil the dark side of proteome evolution.

However, we investigated how residual differences of protein conservation can be explained by those features and properties after controlling for abundance.

Finally, we analyzed it within different protein categories which correspond to lists of related properties and features (Sequence, Biophysics, Phenotypes...).

## Definitions

A protein feature is a measurable quantity related to its activity, composition... The numerical values will be stored in a vector as a column of proteome data table. (e.g. disorder content, sequence length)

A protein property is a shared non-quantitative trait among a subset of proteins. A vector named after the common characteristic will store the identifiers (ORF, UNIPROT...) of the subset members. (e.g. localization, pathway, function...)

Finally, a protein category correspond to a list of features or properties (Sequence, Biophysics, Phenotypes...) Quantitative variables will be aggregated in one Proteome table (rows=proteins, columns=variables)

# Datasets

## Reference proteome sequence

SGD and UniProt are the most reliable database gathering information about genes and proteins sequences. SGD is the main provider for the assembled genome of S. cerevisiae as well as annotations about the genes.
...
#ortho_stat = count_taxons_eggnog_node(node = 4751)
...
#test=  PPM.ORTHO.4932 %>% add_count(NOG) %>% dplyr::filter(n>1) %>% group_by(NOG) %>% summarise(ppm.avg=median(ppm_4932),paxdb.ppm_4896) %>% distinct()
#scor(test$ppm.avg, test$paxdb.ppm_4896)

# VARIABLE ORF (PANGENOMICS) -------------------------------------------------
VARORF = load.peter2018.data(2)
file.r4s.fungi_snp=here("data","PROTEIN-EVO-FUNGI-SNP.rds")
SNP_STRAIN = readRDS(file.r4s.fungi_snp) %>% distinct(ORF,UNIPROT,.keep_all=T) %>%
...
AA.FR = letterFrequency(S288C,as.prob = T,letters = get.AA1()) %>% bind_cols( id=names(S288C)) 
AA.COUNT = letterFrequency(S288C,as.prob = F,letters = get.AA1())%>% bind_cols( id=names(S288C)) 
...
AACLASS.FR = map(aa.prop, sum.aa.fr, BS=S288C )
...
TM = load.jarzab2020.data()

## MRNA HALFLIFE  --------------------------------------------------------------
MRNA.HL = load.geisberg2014.data() 
## PROTEIN HALFLIFE ------------------------------------------------------------
christiano2014 =  load.christiano2014.data() %>% 
                    separate_rows('ensg',sep=';') %>% 
                    filter(ensg %in% PROTEOME$ORF & uniprot %in% PROTEOME$UNIPROTKB ) %>% 
                    distinct() %>% arrange(uniprot,ensg) %>% 
                    group_by(ensg) %>% mutate(degrate_mn=mean_(degradation_rates_min_1),
                                              t_half_min = mean_(t1_2_min),
                                              t1_2_hr = mean_(t1_2_hours )) %>%
                    dplyr::select(-c(degradation_rates_min_1,t1_2_min, t1_2_hours,
                                     r2_quality_of_curve_fitting, cv_of_slope))

PROT.HL = load.belle2006.data() %>% 
          left_join( load.villen2017.data(), by = c('ORF')) %>%
          left_join( christiano2014, by=c('ORF'='ensg'))
...
CYC = load.pu2008.data() %>% # BASED ON ORF
...
-   DNA composition
        ...
        * GC content
            The genomic sequence made of DNA molecules is composed of 2 pairs of complementary nucleic acids called bases (AT & CG). Variations in usage of both pairs exist.
...
df.pangenome =  VARORF %>%
                ...
                mutate(peter2018.f_gene_loss =(1011-peter2018.strains_presence)/1011 )

GENOMICS$peter2018.pangenome_rare= df.pangenome %>% filter(peter2018.strains_presence<435) %>% pull(orf.s288c) # Removed from the pangem
GENOMICS$peter2018.pangenome_cloud = df.pangenome %>% filter(between(peter2018.strains_presence,435,750)) %>% pull(orf.s288c)
...
-   Codon Usage
        ...
        * tRNA adapation
            Some codons are faster to translate than others, even when both code for the same amino acid, i.e. synonymous codons. The copy number of tRNA genes determine how fast tRNA can load the amino acid corresponding to a particular codon. Genes optimized for translation are likely to use codons for which tRNA genes have high copy number.

        * codon usage
            Other measures of codon usage are commonly used but they often require to compare to a set of optimized genes. Generally, ribosomal genes are used as the set of reference to.

-   Amino acid biosynthetic cost

        * Molecular weight per AA
            The average amino acid molecular weight of a particular protein sequence informs on how cost-effective is its production.

        * Consumption of ATP per AA
            Other measure of biosynthetic cost of amino acid are often used. For example, by measuring how many ATP molecules are required to synthetize each amino acid.

```{r transcriptomics}
TRANSCRIPTOMICS=list()

df.rates = load.hausser2019.data() %>% 
  dplyr::rename_with(.fn=Pxx,px='hausse2019.rates',s='_',.cols=-gene)

### Translation initiation
#df.tie = load.szavitsnossan2020.data() %>% 
#  rename_with(.fn=Pxx, px='szavitsnossan2020.TIE',s='_',.cols=-orf)
### Codon adaptation
df.tai=load.trna.adaptation(inputseq=CDS, 
                            trnas = here::here('data','GtRNA-counts.rds')) %>% 
  dplyr::rename(codonr.tAI = tAI)
...
codon_count = Biostrings::trinucleotideFrequency(CDS,step = 3) %>% as_tibble
col_codons=codon_table$codon_aa[codon_table$CODON == colnames(codon_count)]
names(codon_count) = col_codons

df.codons = bind_cols(orf=names(CDS), codon_count, len_cds=lengths(CDS)) %>%             
            dplyr::rename_with(.fn=Pxx,px="sgd")
 
### Expression in different conditions
df.cond_exp = load.lahtvee2017.data() %>%  dplyr::rename(orf=mrna_geneId) %>%
              rename_with(.fn=Pxx, px='lahtvee2017.exp',s='_',.cols=-orf)

exp_var = df.cond_exp %>% group_by(orf) %>% 
  summarise(var_mrna= var_(c_across(contains("pg_mrna"))),
            var_prot= var_(c_across(contains("log10.pg_prot"))),
            var_trans = var_(c_across(contains("log10.trans_")))) %>%
  dplyr::filter(!is.na(var_mrna) | !is.na(var_prot) | !is.na(var_trans))
  

TRANSCRIPTOMICS$lahtvee2017.mrna_hivar = exp_var %>% slice.d90(var_mrna) %>% pull(orf)
TRANSCRIPTOMICS$lahtvee2017.mrna_lovar = exp_var %>% slice.d10(var_mrna) %>% pull(orf)
TRANSCRIPTOMICS$lahtvee2017.prot_hivar = exp_var %>% slice.d90(var_prot) %>% pull(orf)
TRANSCRIPTOMICS$lahtvee2017.prot_lovar = exp_var %>% slice.d10(var_prot) %>% pull(orf)
TRANSCRIPTOMICS$lahtvee2017.trans_hivar = exp_var %>% slice.d90(var_trans) %>% pull(orf)
TRANSCRIPTOMICS$lahtvee2017.trans_lovar = exp_var %>% slice.d10(var_trans) %>% pull(orf)
...
df.orthoexp =  PPM.ORTHO.4932 %>%
                 group_by(NOG,orf) %>%
                 mutate( paxdb.ortho_avg_log10 = mean_(log10(c_across(starts_with('paxdb.ppm_')))),
                         paxdb.ortho_sd_log10  = sd_(log10(c_across(starts_with('paxdb.ppm_')))),
                         paxdb.ortho_cv_log10  = paxdb.ortho_sd_log10/paxdb.ortho_avg_log10,
                         paxdb.ortho_max_log10  = max_(log10(c_across(starts_with('paxdb.ppm_')))),
                         paxdb.ortho_min_log10  = min_(log10(c_across(starts_with('paxdb.ppm_')))),
                         paxdb.ortho_ppm_n= sum.na(c_across(starts_with('paxdb.ppm_')),notNA = T) ) %>%
                  filter(paxdb.ortho_ppm_n > 1) %>%  dplyr::select(-ppm_4932) %>% 
                  mutate(across(starts_with("paxdb.ppm_"),log10,.names="{.col}_log10")) %>%
                  dplyr::select(-c(paxdb.ppm_4896,paxdb.ppm_5476,paxdb.ppm_214684,paxdb.ppm_5061))
    
df.transcriptomics = PROTEOME %>%
                    left_join(df.rates,by=c('GNAME'='gene')) %>%
                    #left_join(df.tie,by=c('ORF'='orf')) %>%
                    ...
                    left_join(df.cond_exp,by=c('ORF'='orf'))
...
df.aa = AA.FR %>%
        rename_with(.fn = Pxx, px="sgd.f",s='_',.cols=-id)
...
superfamilies = split(SUPERFAM$seqid,SUPERFAM$superfamily_description) %>% map(unique) %>% keep(function(x){ n_distinct(x)> NMINI }) 
...
pfam_clans = split(pfam$seq_id,pfam$clan_name) %>% map(unique) %>% keep(function(x){ n_distinct(x)> NMINI }) 
...
df.tm = TM %>% 
        ...
        rename_with(.fn=Pxx, px='jarzab2020.TPP',s='_',.cols=-protein_id)
BIOPHYSICS$jarzab2020.Tm_low = TM %>% dplyr::filter( Tm_type == 'Low-Tm' ) %>% pull(UNIPROT)
BIOPHYSICS$jarzab2020.Tm_med = TM %>% dplyr::filter( Tm_type == 'Medium-Tm' ) %>% pull(UNIPROT)
BIOPHYSICS$jarzab2020.Tm_hi = TM %>% dplyr::filter( Tm_type == 'High-Tm' ) %>% pull(UNIPROT)
BIOPHYSICS$jarzab2020.Tm_nomelt = TM %>% dplyr::filter( Tm_type == 'Non-melter' ) %>% pull(UNIPROT)

df.HLprot = PROT.HL %>%
            dplyr::select(ORF,halflife.avg,halflife_mn_corrected,t_half_min,degrate_mn) %>%
            dplyr::rename(villen2017.HL_prot=halflife.avg,
                          belle2006.HL_prot=halflife_mn_corrected,
                          christiano2014.HL_prot = t_half_min,
                          christiano2014.deg_rate = degrate_mn)
...
BIOPHYSICS$christiano2014.longlived_prot = df.HLprot %>% dplyr::filter(christiano2014.HL_prot > 900) %>% pull(ORF)

BIOPHYSICS$christiano2014.shortlived_prot = df.HLprot %>% dplyr::filter(christiano2014.HL_prot < 150) %>% pull(ORF)
...
BIOPHYSICS$christiano2014.slow_turnover = df.HLprot %>% dplyr::filter(get.d10(christiano2014.deg_rate)) %>% pull(ORF)
BIOPHYSICS$christiano2014.fast_turnover = df.HLprot %>% dplyr::filter(get.d90(christiano2014.deg_rate)) %>% pull(ORF)

df.biophysics = PROTEOME %>% 
                left_join(df.aa,by=c('ORF'='id')) %>%
                left_join(df.aa_class,by=c('ORF'='id')) %>%
                ...
                left_join(df.tm,by=c('UNIPROTKB'='UNIPROT'))
)
...
CYC_machines = keep(CYC_complexes,function(X){ length(X) > NMINI  })
...
EBI_machines = keep(EBI_complexes,function(X){ length(X) > NMINI  })
...
unigo  = UNIGO %>% # get goterm from BP and MF not obsolete and with 30-1000 proteins annotated
         ...
         filter(!obsolete & shared > NMINI & ONTOLOGY != "CC")
...
biofunc = load.vanleeuwen2016.data(single_orf = F)
func_class = split(biofunc$ORF,biofunc$BIOPROCESS)
...
kegg_path   = keep(split(PATHWAY$id,PATHWAY$desc),function(X){ length(X) > NMINI  })
names(kegg_path) = paste0("keggpath.KO_", make_clean_names(names(kegg_path), case='none') )
kegg_module = keep(split(MODULE$id,MODULE$desc),function(X){ length(X) > NMINI  })
...
-   Essentiality vs. Dispensability
    ...
    As their name suggest, essential genes are required for the cell to be viable. Inherited or acquired mutations in those genes can lead to pathological outcomes. However, a synthetic rescue can happen when the phenotypic outcome created by primary mutation is alleviated by suppressor mutations. Hence, those genes are dubbed dispensable (essential until bypass suppression).
...
signatures= keep(split(CHEMSIG$gene,CHEMSIG$response_signature), function(X){ length(X)>NMINI })
...
saveRDS(FEATURES,here::here("output", sprintf("proteome-features-%s.rds",when)))
...
saveRDS(PROP,here::here("output",sprintf("proteome-properties-%s.rds",when)))
save.image(here::here("output",sprintf("loaded-datasets-%s.rds",when)))
...
source(here::here("src","theme_black.r"))
...
BP = ggplot(count.cat, aes(x=categories.f, y=N, label=N, fill=categories.f)) + 
  geom_col() +
  geom_text(col='white',size=8,hjust='inward') + facet_wrap(~type,scales = 'free_x') + 
...
save_plot(plot = BP, filename = here::here("output","S0.prop-feat-count.png"), base_height = 8, base_width = 8)
save_plot(plot = bp0, filename = here::here("output","S0_1.prop-count.png"), base_height = 8, base_width = 8)
save_plot(plot = bp1, filename = here::here("output","S0_2.feat-count.png"), base_height = 8, base_width = 8)
...
**`r TOTAL_FEATURES`** protein features were gathered.</br> Here are the breakdown of features by categories: `r knitr::kable(FEATURE_BY_CATEGORIES)`

**`r TOTAL_PROPERTIES`** proteome properties were retrieved. Here are the breakdown of properties by categories:
