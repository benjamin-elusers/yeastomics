...
.res_prop = propfit %>% 
              group_by(col_prop) %>% 
              mutate(  N=n(),
                .res_evo.RS.pc  = sum(.resid.evo)/TSS.evo,
                ...
                .res_snp.RS.pc  = sum(.resid.snp)/TSS.snp,
  ...
  arrange(desc(.res_evo.RS.pc),desc(.res_snp.RS.pc)) %>%
  dplyr::select(categories,source,property, col_prop, N, starts_with('.res_')) %>%
  distinct() %>% 
  group_by(categories) %>% mutate(rRS = dense_rank(.res_evo.RS.avg) , ncat=n())
...
YMIN = min(-0.2,propsubres$.res_evo.RS.pc)%>% round(digits = 1)
YMAX = max(0.2,propsubres$.res_evo.RS.pc) %>% round(digits = 1)
p1c =  ggplot(propsubres,
             aes(x = reorder(str_trunc(property,w=50), -.res_evo.RS.pc),
                 y = .res_evo.RS.pc, fill=property), group=source) +
...
F3 = ggplot(.res_prop %>% slice_min_max( order_by=.res_evo.RS.pc, n=10 ), 
               aes(y = reorder(str_trunc(property,w=50), -.res_evo.RS.pc),
                   x = .res_evo.RS.pc, fill=source)) +
      geom_bar(stat='identity',position='dodge')  + 
...
mu=mean_(STRAINS$EVO.FULL)
s = sd_(STRAINS$EVO.FULL)
md=median_(STRAINS$EVO.FULL)
rg = range_(STRAINS$EVO.FULL)

nsd = 2
symmetry = function(x){ return( sort(c(x,x)) * (-1)^(1:length(x)) ) }
sd_x = mu + s* symmetry(x=1:nsd)

D=density(STRAINS$EVO.FULL,bw=0.1,na.rm=T)
...
A = ggplot(subset(STRAINS,!is.na(EVO.FULL))) + 
          ggpubr::grids() +
...
selected.prop = c('MF_nucleotide_binding','MF_molecular_function','essential_core','essential_dispensable','peter2018.pangenome_rare','peter2018.pangenome_cloud')
ER.prop = propfit %>% dplyr::filter(property %in% selected.prop) %>% dplyr::select('EVO.FULL','.resid.evo')
