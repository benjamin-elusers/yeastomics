...
validation = all_orthologs %>% filter( !(uniprot %in% HS_PPM$uniprot) )
orthologs =  all_orthologs %>% filter(uniprot %in% HS_PPM$uniprot)
dim(validation)
dim(orthologs)


sum(HS_DATA_pred2$uniprot[is.na(HS_DATA_pred2$uniprot.GC_cdna)] %in% hs_uniref)
