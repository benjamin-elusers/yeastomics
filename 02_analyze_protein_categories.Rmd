...
prop = ER.PROP %>% dplyr::select(starts_with("cat_"))
under10 = colSums(prop)<10
...
ggplot(prop.size) + 
  geom_bar(aes(y=size.prot, x=reorder(property,size.prot),fill=source),stat='identity') +
  geom_bar(aes(y=size.ortho, x=reorder(property,size.prot)),stat='identity') + 
