...
prop.size = prop %>% 
  group_by(ORF,categories,source,property) %>% 
  summarise( in_prot=sum(has_prop), 
             in_ortho=sum(has_prop*has_ortho),
             r_prot_ortho = in_ortho/in_prot)
  
under10 = colSums(prop[,-1])<10
sum(under10); length(prop)-sum(under10)
# 
# prop.size= 
#   tibble( 
#         prop.name  = names(prop[under10]),
#         size.ortho = colSums(prop[,under10]),
#         size.prot  = colSums(PROP[,prop.name]),
#         size.ratio = size.ortho/size.prot
#       ) %>% 
#         arrange(size.prot) %>%
#         separate(prop.name, 
#                 into = c('categories','source','property'), 
#                 sep="\\.")
...
ggplot(prop.size) + 
  ...
  geom_hline(yintercept=10,color='white',linetype='dashed') +
  facet_wrap(~categories,scales = 'free_y') +
  coord_flip() + theme( axis.text = element_text(size=5))


prop.1 = prop[,c(1,which(!under10)+1)]

prop.2 = proxy::dist(prop.1[,-1], prop.1[,-1], by_rows=F, method="Jaccard")

prop.1.1 = prop.1 %>% pivot_longer(cols = -GNAME,
                                   names_to = c('categories','source','property'), 
                                   names_pattern="cat_(.+)\\.(.+)\\.(.+)",
                                   values_to = "has_prop")

#heatmap(x=as.matrix(1*prop.1), Rowv = F, Colv = F, symm = F)
ggplot(prop.1.1) + 
  geom_raster(aes(x=GNAME,y=property),show.legend = F,interpolate=T,color=NA) +
  scale_fill_manual(values=c('black','white')) +
  theme(legend.title = element_blank(), axis.text = element_blank())

pheatmap::pheatmap(prop.1[,-1]*1, 
