...
>>>>>>> d3dc28a9bb5e2584e010701cf33be7dc63b277ca
...
INTACT_ppi.0 = read.csv("/media/elusers/users/hugo/07_3DComplex_scripts/PPIs_analysis/INTACT/PPIs_cerevisiae.txt",sep = "\t", quote = "", stringsAsFactors = F)
...
cat("nb PPIs before rm EBI: ", nrow(INTACT_ppi.0), "\n")
INTACT_ppi.1 = INTACT_ppi.0[!(grepl("EBI", INTACT_ppi.0$protA) | grepl("EBI", INTACT_ppi.0$protB)),]
cat("nb PPIs after rm EBI: ", nrow(INTACT_ppi.1), "\n")
...
##### STRING network
STRING_ppi = load.STRING(tax="4932",phy=T, ful=T, min.score = 700) %>% 
...
STRING_net = as_tbl_graph(STRING_ppi)
...
pheatmap::pheatmap(M,show_colnames = F,show_rownames = T,cluster_cols = T,cluster_rows = T, fontsize = 4)
                   #filename = "GO_profile_spearman.pdf", height = 18, width=10)
pheatmap::pheatmap(ol_scaled,show_colnames = F,show_rownames = T,cluster_cols =T,cluster_rows = T, fontsize = 4)
                   #filename = "GO_profile_overlap.pdf", height = 18, width=10)
go =  split(unigo$UNIPROT,unigo$go)
PHENOTYPES = append(PHENOTYPES,go)

test = load.yeast.biofunctions()
table( test$Function )
