...
chr.1let = paste0("chr_",c(NA,LETTERS[1:2],"2-mu",LETTERS[3:16],"mito"))
...
AA.FR = letterFrequency(UNI.SC,as.prob = T,letters = get.AA1()) %>% as_tibble
...
aa.class = paste0(tolower(names(aa.set)),"_",aa.set)
AACLASS.FR = map(aa.prop, sum.aa.fr, BS=UNI.SC )

# DISORDER (D2P2 + IUP)  -------------------------------------------------------
file.d2p2="data/d2p2-yeast-uniprotKB.rds"
D2P2 = get.d2p2.diso(load.d2p2(saved=file.d2p2),as.df = T) %>%
...
file.intact = "data/intact-yeast-ppi.rds"
...
CYC = load.pu.2008.data() %>% # BASED ON ORF
      mutate(oligomers = cut(n_members, breaks = c(1:10,20,81), 
                             labels =  paste0("CYC.",c(nmers[2:10],'high_oligomer','molecular_machine'))) )
## COMPLEXTAB = Complex Portal from EBI ----------------------------------------
CPX = load.meldal.2019.data() %>%
  filter(is_uniprot) %>% # BASED ON UNIPROT
      mutate(oligomers = cut(n_members, breaks = c(1:10,20,81), 
                             labels = paste0("CPX.",c(nmers[2:10],'high_oligomer','molecular_machine'))),
...
file.chemofit= "data/lee2014-chemogenomic-fitness-defect-heterozygous.rds"
...
### Gene duplication ----------------------------------------------------------
df.wgd  = OHNO %>% dplyr::select(orf,dup.orf,RLEN,PID1,pid,SCORE.B100,S,N,G)
GENOMICS$byrne2005.wgd_all = OHNO$orf 
GENOMICS$byrne2005.wgd_sym = OHNO %>% filter(RLEN>0.99) %>% pull(orf)
GENOMICS$byrne2005.wgd_asym = OHNO %>% filter(between(RLEN,0.7,0.9)) %>% pull(orf)
GENOMICS$byrne2005.wgd_trunc = OHNO %>% filter(RLEN<0.5) %>% pull(orf)
### Pan genome
df.pangenome =  VARORF %>%
...
df.aaMW = DUB %>% 
       group_by(UNIPROT) %>% 
       summarise( pepstats.AA_MW = MW/prot.size ) 

TRANSCRIPTOMICS$pepstats.costly = df.aaMW %>% dplyr::filter(pepstats.AA_MW > 120 ) %>% pull(UNIPROT)
TRANSCRIPTOMICS$pepstats.cheap = df.aaMW %>% dplyr::filter(pepstats.AA_MW < 105 ) %>% pull(UNIPROT)

df.cost = load.barton2010.data("prot") %>% 
          group_by(name) %>%
          mutate(rna_exp = mean(transcript_level)) %>%
          dplyr::select('name','trna','transcript_level','cai','rna_exp',
...
df.transcriptomics = PROTEOME %>% 
                    ...
                    left_join(df.aaMW,by=c('UNIPROTKB'='UNIPROT')) %>% 
...
df.aa = AA.FR %>% mutate(uni=names(UNI.SC)) %>% relocate(uni) %>%
        rename_with(.fn = Pxx, px="uniprot.f",s="_")
...
df.d2p2 = D2P2 %>% dplyr::rename(d2p2.id = d2p2_id) %>%
          dplyr::filter(has.d2p2) %>%
          dplyr::select(-c(has.d2p2,d2p2.size)) %>%
          group_by(d2p2.id) %>% 
          summarise(d2p2.L = sum_(d2p2_diso>=7), 
                 d2p2.f = mean_(d2p2_diso>=7),
                 d2p2.nseg = n_distinct(d2p2_seg),
                 d2p2.Lsegmax = max(d2p2_seglen))
...
df.iup = DUB %>% mutate(has_iup = !is.na(L.IUP20) | !is.na(L.IUP30) | !is.na(L.IUP40)  ) %>%
         ...
         rename_with(.fn=str_replace, pattern="(.)\\.(.+)",replacement = '\\2.\\1')
BIOPHYSICS$iupred.standard = DUB %>% dplyr::filter(standard) %>% pull(UNIPROT)
BIOPHYSICS$iupred.medium= DUB %>% dplyr::filter(medium) %>% pull(UNIPROT)
BIOPHYSICS$iupred.high= DUB %>% dplyr::filter(high) %>% pull(UNIPROT)
BIOPHYSICS$topcons.membrane= DUB %>% dplyr::filter(isMB) %>% pull(UNIPROT)

BIOPHYSICS$uniprot.small= DUB %>% dplyr::filter(small) %>% pull(UNIPROT)
BIOPHYSICS$uniprot.average= DUB %>% dplyr::filter(average) %>% pull(UNIPROT)
BIOPHYSICS$uniprot.large= DUB %>% dplyr::filter(large) %>% pull(UNIPROT)
...
BIOPHYSICS$superfam.none   = setdiff(PROTEOME$ORF,supfam$seqid)
BIOPHYSICS$superfam.single = supfam %>% filter(superfamily.ndom==1) %>% pull(seqid)
BIOPHYSICS$superfam.pair   = supfam %>% filter(superfamily.ndom==2) %>% pull(seqid)
BIOPHYSICS$superfam.multi  = supfam %>% filter(superfamily.ndom>=3) %>% pull(seqid)
superfamilies = split(SUPERFAM$seqid,SUPERFAM$superfamily_description) %>% map(unique) %>% keep(function(x){ n_distinct(x)>9 }) 
names(superfamilies) = paste0("superfamilies.",janitor::make_clean_names(names(superfamilies)))
...
BIOPHYSICS$pfam.none   = setdiff(PROTEOME$UNIPROTKB,pfam$seq_id)
BIOPHYSICS$pfam.single = pfam %>% filter(pfam.ndom==1) %>% pull(seq_id)
BIOPHYSICS$pfam.pair   = pfam %>% filter(pfam.ndom==2) %>% pull(seq_id)
BIOPHYSICS$pfam.multi  = pfam %>% filter(pfam.ndom>=3) %>% pull(seq_id)
pfam_clans = split(pfam$seq_id,pfam$clan_name) %>% map(unique) %>% keep(function(x){ n_distinct(x)>9 }) 
names(pfam_clans) = paste0("pfamclan.",janitor::make_clean_names(names(pfam_clans)) )
BIOPHYSICS = append(BIOPHYSICS,pfam_clans)

### Protein stability  ------------------------------------------------------------
df.dg= DELTAG %>% 
       dplyr::select(Protein_ID,tmprot=Tm.Protein,protcov=Protein.Coverage,npep,nres) %>%
  rename_with(.fn=Pxx, px='LIP2017')
      
BIOPHYSICS$leueunberger2017.stable = DELTAG %>% dplyr::filter( Protinfo == "Stable") %>% pull(Protein_ID)
BIOPHYSICS$leueunberger2017.medium = DELTAG %>% dplyr::filter( Protinfo == "Medium") %>% pull(Protein_ID)
BIOPHYSICS$leueunberger2017.unstable = DELTAG %>% dplyr::filter( Protinfo == "Unstable") %>% pull(Protein_ID)
...
df.biophysics = PROTEOME %>% 
                left_join(df.aa,by=c('UNIPROTKB'='uniprot.f_uni')) %>%
                left_join(df.aa_class,by=c('UNIPROTKB'='uniprot.f_id')) %>%
                ...
                left_join(df.dg,by=c('UNIPROTKB'='LIP2017.protein_id'))
...
cent.STRING = network.centrality(STRING[,c('ORF1','ORF2')])
df.STRING_cent =  cent.STRING %>% rename_with(~Pxx(x=., px="string",s='.'), .cols=-ids)
quantile(cent.STRING$cent_deg)

INTERACTIONS$string.megahub = cent.STRING %>% dplyr::filter(cent_deg>300) %>% pull(ids)
INTERACTIONS$string.superhub = cent.STRING %>% dplyr::filter(between(cent_deg,100,300)) %>% pull(ids)
...
cent.INTACT = network.centrality(INTACT[,c('protA','protB')] %>% distinct() )
df.INTACT_cent = cent.INTACT %>% rename_with(~Pxx(x=., px="intact",s='.'), .cols=-ids)
quantile(cent.INTACT$cent_deg)
INTERACTIONS$intact.megahub = cent.INTACT %>% dplyr::filter(cent_deg>50) %>% pull(ids)
INTERACTIONS$intact.superhub = cent.INTACT %>% dplyr::filter(between(cent_deg,20,50)) %>% pull(ids)
INTERACTIONS$intact.minihub = cent.INTACT %>% dplyr::filter(between(cent_deg,10,20)) %>% pull(ids)

# PROTEIN COMPLEXES -----------------------------------------------------------
## Catalogue of Yeast Complexes (CYC)
CYC_complexes = split(CYC$ORF,CYC$Complex)
names(CYC_complexes) = paste0("CYC.",janitor::make_clean_names(names(CYC_complexes)))
...
names(EBI_complexes) = paste0("EBI.",janitor::make_clean_names(names(EBI_complexes)))
...
df.interactions = PROTEOME %>%
                  ...
                  left_join(df.INTACT_cent, by=c("ORF"="ids"))
...
names(go) = paste0("go.", make_clean_names(names(go)) )

FUNCTIONS=append(FUNCTIONS,go)

# Classification of Biological functions
biofunc = load.vanleeuwen2016.data() %>% mutate( FUNCTION = janitor::make_clean_names(FUNCTION))
func_class = split(biofunc$ORF,biofunc$FUNCTION)
names(func_class) = paste0("costanzo2010.", make_clean_names(names(func_class)) )
...
names(kegg_path) = paste0("keggpath.", make_clean_names(names(kegg_path)) )
kegg_module = keep(split(MODULE$id,MODULE$desc),function(X){ length(X) > 9  })
names(kegg_path) = paste0("keggmod.",make_clean_names(names(kegg_module)) )
...
PHENOTYPES$vanleeuwen2020.essential   = essential
PHENOTYPES$vanleeuwen2020.dispensable = dispensable
...
FEATURES = PROTEOME %>% 
            left_join(df.genomics, by=KEYS) %>%
            left_join(df.transcriptomics, by=KEYS) %>%
            left_join(df.biophysics, by=KEYS) %>%
            left_join(df.interactions, by=KEYS) 
...
TOTAL_CATEGORIES = sapply(CATEGORIES,length)
TOTAL_PROPERTIES = length(flatten(CATEGORIES))

#sort( sapply(flatten(CATEGORIES),n_distinct) )
PROPERTIES= unlist(CATEGORIES,rec=F)

###mini.properties = PROPERTIES[sapply(PROPERTIES,n_distinct) < 10]
###mega.properties = PROPERTIES[sapply(PROPERTIES,n_distinct) > 1000]

# Make a proteome table of all properties
PROT = PROTEOME
for( C in names(PROPERTIES)){
  PROT[,C] = (PROT$UNIPROTKB %in% PROPERTIES[[C]]) | (PROT$ORF %in% PROPERTIES[[C]])
}
head(PROT)

M = cor(PROT[,-c(1:5)], use = 'pairwise.complete',method = 'spearman') # similarity between properties
pheatmap::pheatmap(M,show_colnames = F,show_rownames = T,cluster_cols = T,cluster_rows = T, border_color = NA,
                   filename = "plots/properties-overlap.pdf", height=50, width=50, cellheight=5, cellwidth=5,fontsize=6)

when=timestamp(stamp = format(Sys.time(), "%d_%m_%Y"),prefix = "", suffix = "")
saveRDS(PROT,sprintf("data/proteome-properties-%s.rds",when))
saveRDS(FEATURES,sprintf("data/proteome-features-%s.rds",when))
...
So far, **`r TOTAL_PROPERTIES`** proteome properties were retrieved within the following categories:
...
```{r fit-ER, eval=FALSE, include=FALSE}
...
MR = ggplot(fit.ER, aes(y = EVO.FULL, x = PPM)) +
     ...
     geom_point(data=subset(fit.ER,essential),color='purple',fill=NA,shape=21,size=2)+
...
MR2= ggplot(data=fit.ER, aes(y=.resid,x=PPM)) +
  ...
  geom_point(data=subset(fit.ER,essential),color='purple',fill=NA,shape=21,size=2)+
  geom_segment(data=subset(fit.ER,essential),aes(xend = PPM, y=0, yend = .resid),col='purple') +
...
RES.CAT =res.cat %>% filter(abs(sum.resid)>10)
fig.1 = ggplot(data=RES.CAT, aes(y=reorder(filter, -sum.resid),x=sum.resid, label=filter)) + 
  geom_bar_text(position = "dodge",grow=T, reflow=T, place='left', fullheight=F) +
  theme_minimal()


ggsave(fig.1,path = "plots/", scale=3,filename = "_PROPERTIES_1.evo_rate_properties.png")
