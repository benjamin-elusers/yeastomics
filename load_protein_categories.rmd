...
get.q75  = function(x,...){ dplyr::slice_max(x,prop=0.75,with_ties = T,...) }
get.iqr  = function(x,...){ dplyr::filter(between(!!(x),q25(!!(x)),q75(!!(x)))) } 
...
GENOMIC$average  = get.iqr(x=len) %>% pull(orf)
...
GENOMIC$GC_mid    = df.gc %>% get.iqr(pGC) %>% pull(orf)
