...
OHNO = get.sc.ohno(myseq=S288C) %>%
        left_join(PROTEOME %>% rename_all(list(~xxS(x=.,sx='ref'))), by=c('orf'='ORF.ref')) %>%
        left_join(PROTEOME %>% rename_all(list(~xxS(x=.,sx='dup'))), by=c('dup.orf'='ORF.dup'))
dim(OHNO)
GENOMICS$WGD_dup = OHNO$orf 
GENOMICS$WGD_dup_ = OHNO %>% filter(RLEN>0.99) %>% pull(orf)
...
#### Amino-acid interactins propensities
df.scales = DUB %>% dplyr::select(UNIPROT,ends_with(c('.dom','.iup20'),ignore.case = F))
#### Amino-acid biosynthetic cost
AAMW = DUB %>% 
       group_by(UNIPROT) %>% 
       summarise( AA = MW/prot.size ) %>% 
       left_join(PROTEOME,by=c("UNIPROT"="UNIPROTKB"))
df.cost = load.barton2010.data("prot") %>% left_join(AAMW, by=c('name'='ORF'))

### PROPERTIES (QUALITATIVE OR SEMI-QUANTITATIVE)
#### Protein domains (nodom,single,multi)  ----------------------------------------
superfam=load.superfamily() 
SUPFAM =  superfam %>% 
...
pfam=load.pfam() %>% group_by(clan) %>% mutate(np_clan = n_distinct(seq_id))

split(pfam$seq_id,pfam$clan)
PFAM= pfam %>% 
...
QUANTITATIVE = PROTEOME %>% 
               ...
               left_join(df.cost,by=c('UNIPROTKB'='UNIPROT'))
