...
INTACT = preload(saved.file = file.intact, loading.call = load.intact.yeast(min.intact.score = 0.75), doing = "loading PPIs from INTACT...")
...
cent.STRING = network.centrality(STRING)
df.STRING_cent =  cent.STRING %>% rename_with(~Pxx(x=., px="STRING",s='_'), .cols=-ids)
## INTACT protein-protein physical interaction network
cent.INTACT = network.centrality(INTACT)
df.INTACT_cent = cent.INTACT %>% rename_with(~Pxx(x=., px="INTACT",s='_'), .cols=-ids)
