...
#### Genome arrangement (duplication, introns, chromosomes)
#### DNA composition (GC content, coding length)
...
### Gene duplication ----------------------------------------------------------
GENOMIC$WGD = OHNO$orf 
GENOMIC$WGD_const.size = OHNO %>% filter(RLEN>0.99) %>% pull(orf)
```

### Transcriptomics category

#### Codon Usage (tRNA adapation, Fop, codons counts)
#### Amino acid biosynthetic cost (average MW of AA, ATP consumption...)
...
#### Amino acid compositions (frequencies of single and classes of AA)
#### Intrinsic Disorder in proteins (IUP, D2P2)
...
df.scales = DUB %>% dplyr::select(UNIPROT,ends_with(c('.dom','.iup20'),ignore.case = F))
...
BIOPHYSICS$supfam_none   = setdiff(PROTEOME$ORF,supfam$seqid)
...
pfam_clans = split(pfam$seq_id,pfam$clan)
...
#preload(file.intact, loading.call = load.intact.yeast(), doing = "loading PPIs from INTACT...")
...
GENOMIC$shortest = get.d10(QUANTITATIVE,order_by = len) %>% pull(ORF)
GENOMIC$shorter  = get.q25(QUANTITATIVE,order_by = len) %>% pull(ORF)
GENOMIC$average  = get.iqr(QUANTITATIVE,x=len,lo=0.4,up=0.6) %>% pull(ORF)
GENOMIC$longest  = get.q75(QUANTITATIVE,order_by = len) %>% pull(ORF)
GENOMIC$longest  = get.d90(QUANTITATIVE,order_by = len) %>% pull(ORF)
#### range of GC content -------------------------------------------------------
GENOMIC$GC_lower  = get.d10(QUANTITATIVE,order_by=pGC) %>% pull(ORF)
GENOMIC$GC_low    = get.q25(QUANTITATIVE,order_by=pGC) %>%  pull(ORF)
GENOMIC$GC_mid    = get.iqr(QUANTITATIVE,x=pGC,lo=0.4,up=0.6) %>% pull(ORF)
GENOMIC$GC_high   = get.q75(QUANTITATIVE,order_by=pGC) %>% pull(ORF)
GENOMIC$GC_higher = get.d90(QUANTITATIVE,order_by=pGC) %>% pull(ORF)
#### range of % disorder (IUP) -----------------------------------------------------
BIOPHYSICS$IUP_lower  = get.d10(QUANTITATIVE,order_by=f.IUP20) %>% pull(UNIPROTKB)
BIOPHYSICS$IUP_low    = get.q25(QUANTITATIVE,order_by=f.IUP20) %>%  pull(UNIPROTKB)
BIOPHYSICS$IUP_mid    = get.iqr(QUANTITATIVE,x=f.IUP20,lo=0.4,up=0.6) %>% pull(UNIPROTKB)
BIOPHYSICS$IUP_high   = get.q75(QUANTITATIVE,order_by=f.IUP20) %>% pull(UNIPROTKB)
BIOPHYSICS$IUP_higher = get.d90(QUANTITATIVE,order_by=f.IUP20) %>% pull(UNIPROTKB)
#### range of % disorder (D2P2) -----------------------------------------------------
BIOPHYSICS$D2P2_lower  = get.d10(QUANTITATIVE,order_by=f.d2p2) %>% pull(UNIPROTKB)
BIOPHYSICS$D2P2_low    = get.q25(QUANTITATIVE,order_by=f.d2p2) %>%  pull(UNIPROTKB)
BIOPHYSICS$D2P2_mid    = get.iqr(QUANTITATIVE,x=f.d2p2,lo=0.4,up=0.6) %>% pull(UNIPROTKB)
BIOPHYSICS$D2P2_high   = get.q75(QUANTITATIVE,order_by=f.d2p2) %>% pull(UNIPROTKB)
BIOPHYSICS$D2P2_higher = get.d90(QUANTITATIVE,order_by=f.d2p2) %>% pull(UNIPROTKB)
#### range of % disorder (IUP) -----------------------------------------------------
BIOPHYSICS$IUP_lower  = get.d10(QUANTITATIVE,order_by=f.IUP20) %>% pull(UNIPROTKB)
BIOPHYSICS$IUP_low    = get.q25(QUANTITATIVE,order_by=f.IUP20) %>%  pull(UNIPROTKB)
BIOPHYSICS$IUP_mid    = get.iqr(QUANTITATIVE,x=f.IUP20,lo=0.4,up=0.6) %>% pull(UNIPROTKB)
...
PROTEOME.CATEGORIES = PROTEOME
for( C in names(properties)){
  PROTEOME.CATEGORIES[,C] = (PROTEOME.CATEGORIES$UNIPROTKB %in% properties[[C]]) | (PROTEOME.CATEGORIES$ORF %in% properties[[C]])
}
head(PROTEOME.CATEGORIES)

M = cor(PROTEOME.CATEGORIES[,-c(1:5)], use = 'pairwise.complete',method = 'spearman') # similarity between properties
pheatmap::pheatmap(M,show_colnames = F,show_rownames = T,cluster_cols = T,cluster_rows = T, fontsize = 4)
...
fit.ER = broom::augment_columns(x = nls.fit, data = ER) %>% 
           ungroup() %>% 
           mutate(SS.resid = sum(.resid^2), total.resid = sum(.resid),
                  SS.resid.pos= sum(.resid[.resid>0]^2),
                  SS.resid.neg= sum(.resid[.resid<0]^2)) %>%   
          left_join(PROTEOME.CATEGORIES,by=c('UNIPROT'='UNIPROTKB','ORF'='ORF'))
...
MR = ggplot(fit.ER, aes(y = EVO.FULL, x = PPM)) +
     ...
     geom_point(data=subset(fit.ER,shortest),color='purple',fill=NA,shape=21,size=2)+
...
MR2= ggplot(data=fit.ER, aes(y=.resid,x=PPM)) +
  ...
  geom_point(data=subset(fit.ER,shortest),color='purple',fill=NA,shape=21,size=2)+
  geom_segment(data=subset(fit.ER,shortest),aes(xend = PPM, y=0, yend = .resid),col='purple') +
  xlab('Protein abundance (log10 ppm)') + ylab('Residuals (ER)') +theme_clean() 
MR2

# Show most over/under estimated categories

colnames(fit.ER)[1:15]
res.cat = fit.ER %>%
  pivot_longer(cols = 14:ncol(fit.ER),names_to = 'filter') %>% 
  filter(value==T) %>% 
  group_by(filter) %>% 
  summarise( sse=sum(.resid) ) %>%
             #ss.SS = sum(.resid^2)/SS.resid, 
             #ss.avg=mean(.resid^2), 
             #ss.sd= sd(.resid^2), n() ) %>% 
  distinct() %>%
  arrange(desc(ss.avg))
...
ggplot(data=res.cat) + geom_bar(mapping=aes(y=reorder(filter, -s),x=s),orientation = 'y',stat = 'identity')
