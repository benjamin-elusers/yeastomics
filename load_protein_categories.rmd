...
#         left_join(PROTEOME %>% rename_with(.fn = xxS,sx='ref'), by=c('orf'='ORF.ref')) %>%
#         left_join(PROTEOME %>% rename_with(.fn = xxS,sx='dup'), by=c('dup.orf'='ORF.dup'))
...
# PROTEIN STABILITY ------------------------------------------------------------
STABILITY = load.leuenberger2017.data() %>%
...
ESSENTIAL = load.vanleeuwen2020.data() %>% mutate( lethal = (disp=='Core' | (KO_exp =='Indispensable' & disp_score < 0.4 ) ) )
...
### Gene duplication ----------------------------------------------------------
GENOMICS$BYRNE2005.WGD = OHNO$orf 
GENOMICS$BYRNE2005.WGD_const.size = OHNO %>% filter(RLEN>0.99) %>% pull(orf)
```
...
### Codon adaptation
df.tai=load.trna.adaptation(inputseq=CDS) %>% rename(CODONR.tAI = tAI)
df.CU=load.codon.usage(cds=CDS) 

### Amino-acid biosynthetic cost
df.aaMW = DUB %>% 
       group_by(UNIPROT) %>% 
       summarise( PEPSTATS.AA_MW = MW/prot.size ) %>% 
       left_join(PROTEOME[,1:3],by=c("UNIPROT"="UNIPROTKB"))
...
df.iup = DUB %>% dplyr::select(UNIPROT,contains("IUP",ignore.case = F))

### Amino-acid interactions propensities
df.aascales = DUB %>% dplyr::select(UNIPROT,ends_with(c('.dom','.iup20'),ignore.case = F))


### Protein domains (nodom,single,multi)  ----------------------------------------
supfam =  SUPERFAM %>% count(seqid,name="n_dom")
#           group_by(`Sequence ID`) %>% 
#           count(`Model ID`,name='n_uniq') %>%

BIOPHYSICS$SUPERFAMILY.none   = setdiff(PROTEOME$ORF,supfam$seqid)
BIOPHYSICS$SUPERFAMILY.single = supfam %>% filter(n_dom==1) %>% pull(seqid)
BIOPHYSICS$SUPERFAMILY.pair   = supfam %>% filter(n_dom==2) %>% pull(seqid)
BIOPHYSICS$SUPERFAMILY.multi  = supfam %>% filter(n_dom>=3) %>% pull(seqid)

pfam = PFAM %>% 
        group_by(clan) %>% mutate(np_clan = n_distinct(seq_id)) %>%
...
BIOPHYSICS$PFAM.single = pfam %>% filter(n_dom==1) %>% pull(seq_id)
BIOPHYSICS$PFAM.pair   = pfam %>% filter(n_dom==2) %>% pull(seq_id)
BIOPHYSICS$PFAM.multi  = pfam %>% filter(n_dom>=3) %>% pull(seq_id)
...
# PPI NETWORK 
## INTACT physical interaction network
test = load.intact.yeast(from.MACOS = T)

file.intact = "data/intact-yeast-ppi.rds"
INTACT = preload(file.intact, loading.call = load.intact.yeast(from.MACOS = T), doing = "loading PPIs from INTACT...")
#dplyr::select(-c(created,updated,checksumA,checksumB,checksumAB,taxA,taxB))

## STRING predicted interaction network
...
FEATURES = PROTEOME %>% 
               ...
               left_join(df.aaMW,by=c('UNIPROTKB'='UNIPROT', 'ORF', 'SGD')) %>% 
...
BINS$shortest = get.d10(FEATURES,order_by = len) %>% pull(ORF)
BINS$shorter  = get.q25(FEATURES,order_by = len) %>% pull(ORF)
BINS$average  = get.iqr(FEATURES,x=len,lo=0.4,up=0.6) %>% pull(ORF)
BINS$longest  = get.q75(FEATURES,order_by = len) %>% pull(ORF)
BINS$longest  = get.d90(FEATURES,order_by = len) %>% pull(ORF)
#### range of GC content -------------------------------------------------------
BINS$GC_lower  = get.d10(FEATURES,order_by=pGC) %>% pull(ORF)
BINS$GC_low    = get.q25(FEATURES,order_by=pGC) %>%  pull(ORF)
BINS$GC_mid    = get.iqr(FEATURES,x=pGC,lo=0.4,up=0.6) %>% pull(ORF)
BINS$GC_high   = get.q75(FEATURES,order_by=pGC) %>% pull(ORF)
BINS$GC_higher = get.d90(FEATURES,order_by=pGC) %>% pull(ORF)
#### range of % disorder (IUP) -----------------------------------------------------
BINS$IUP_lower  = get.d10(FEATURES,order_by=f.IUP20) %>% pull(UNIPROTKB)
BINS$IUP_low    = get.q25(FEATURES,order_by=f.IUP20) %>%  pull(UNIPROTKB)
BINS$IUP_mid    = get.iqr(FEATURES,x=f.IUP20,lo=0.4,up=0.6) %>% pull(UNIPROTKB)
BINS$IUP_high   = get.q75(FEATURES,order_by=f.IUP20) %>% pull(UNIPROTKB)
BINS$IUP_higher = get.d90(FEATURES,order_by=f.IUP20) %>% pull(UNIPROTKB)
#### range of % disorder (D2P2) -----------------------------------------------------
BINS$D2P2_lower  = get.d10(FEATURES,order_by=f.d2p2) %>% pull(UNIPROTKB)
BINS$D2P2_low    = get.q25(FEATURES,order_by=f.d2p2) %>%  pull(UNIPROTKB)
BINS$D2P2_mid    = get.iqr(FEATURES,x=f.d2p2,lo=0.4,up=0.6) %>% pull(UNIPROTKB)
BINS$D2P2_high   = get.q75(FEATURES,order_by=f.d2p2) %>% pull(UNIPROTKB)
BINS$D2P2_higher = get.d90(FEATURES,order_by=f.d2p2) %>% pull(UNIPROTKB)
#### range of % disorder (IUP) -----------------------------------------------------
BINS$IUP_lower  = get.d10(FEATURES,order_by=f.IUP20) %>% pull(UNIPROTKB)
BINS$IUP_low    = get.q25(FEATURES,order_by=f.IUP20) %>%  pull(UNIPROTKB)
BINS$IUP_mid    = get.iqr(FEATURES,x=f.IUP20,lo=0.4,up=0.6) %>% pull(UNIPROTKB)
