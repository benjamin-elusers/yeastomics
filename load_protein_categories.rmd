...
# DOMAINS  ---------------------------------------------------------------------
PFAM=load.pfam() 
SUPERFAM=load.superfamily() %>% dplyr::rename(seqid = "Sequence ID") 
...
CYC = load.pu.2008.data() %>% # BASED ON ORF
      mutate(oligomers = cut(n_members, breaks = c(1:10,20,81), 
                             labels =  paste0("CYC_",c(nmers[2:10],'high_oligomer','molecular_machine'))) )
## COMPLEXTAB = Complex Portal from EBI ----------------------------------------
CPX = load.meldal.2019.data() %>%
  filter(is_uniprot) %>% # BASED ON UNIPROT
      mutate(oligomers = cut(n_members, breaks = c(1:10,20,81), 
                             labels = paste0("CPX_",c(nmers[2:10],'high_oligomer','molecular_machine'))),
...
## Drug Resistance
file.chemofit= "data/lee2014_chemogenomic_fitness_defect.rds"
CHEM =  preload(saved.file = file.chemofit,
                loading.call = load.lee2014.data(),
...
df.aa = AA.FR %>% mutate(uni=names(UNI.SC)) %>% relocate(uni) %>%
        rename_with(.fn = Pxx, px="UNI.f",s="_")
...
df.d2p2 = D2P2 %>%
          dplyr::filter(has_d2p2) %>%
          dplyr::select(-c(has_d2p2,d2p2_size)) %>%
          group_by(d2p2_id) %>% 
...
BIOPHYSICS$topcons.MB= DUB %>% dplyr::filter(isMB) %>% pull(UNIPROT)

BIOPHYSICS$UNI.small= DUB %>% dplyr::filter(small) %>% pull(UNIPROT)
BIOPHYSICS$UNI.average= DUB %>% dplyr::filter(average) %>% pull(UNIPROT)
BIOPHYSICS$UNI.large= DUB %>% dplyr::filter(large) %>% pull(UNIPROT)
...
supfam =  SUPERFAM %>% dplyr::count(seqid,name="superfamily.ndom")
#           group_by(`Sequence ID`) %>% 
#           count(`Model ID`,name='n_uniq') %>%

BIOPHYSICS$SUPERFAMILY.none   = setdiff(PROTEOME$ORF,supfam$seqid)
BIOPHYSICS$SUPERFAMILY.single = supfam %>% filter(superfamily.ndom==1) %>% pull(seqid)
BIOPHYSICS$SUPERFAMILY.pair   = supfam %>% filter(superfamily.ndom==2) %>% pull(seqid)
BIOPHYSICS$SUPERFAMILY.multi  = supfam %>% filter(superfamily.ndom>=3) %>% pull(seqid)
...
pfam_clans = split(pfam$seq_id,pfam$clan) %>% map(unique) %>% keep(function(x){ n_distinct(x)>9 }) 
names(pfam_clans) = paste0("PFAM.",names(pfam_clans))
...
df.biophysics = PROTEOME %>% 
                left_join(df.aa,by=c('UNIPROTKB'='UNI.f_uni')) %>%
                left_join(df.aa_class,by=c('UNIPROTKB'='UNI.f_id')) %>%
                left_join(df.d2p2,by=c('UNIPROTKB'='d2p2_id')) %>%
...
INTERACTIONS$string.megahub = cent.STRING %>% dplyr::filter(degree>100) %>% pull(ids)
INTERACTIONS$string.superhub = cent.STRING %>% dplyr::filter(between(degree,20,100)) %>% pull(ids)
INTERACTIONS$string.minihub = cent.STRING %>% dplyr::filter(between(degree,10,20)) %>% pull(ids)
...
INTERACTIONS$intact.megahub = cent.INTACT %>% dplyr::filter(degree>100) %>% pull(ids)
INTERACTIONS$intact.superhub = cent.INTACT %>% dplyr::filter(between(degree,20,100)) %>% pull(ids)
INTERACTIONS$intact.minihub = cent.INTACT %>% dplyr::filter(between(degree,10,20)) %>% pull(ids)
...
FUNCTIONS = append(FUNCTIONS,pathways)
FUNCTIONS = append(FUNCTIONS,modules)
...
```{r features}
FEATURES = PROTEOME %>% 
            left_join(df.genomics, by=c(UNIPROTKB,SGD,ORF,GNAME,PNAME)) %>%
            left_join(df.transcriptomics, by=c(UNIPROTKB,SGD,ORF,GNAME,PNAME)) %>%
            left_join(df.biophysics, by=c(UNIPROTKB,SGD,ORF,GNAME,PNAME)) %>%
            left_join(df.interactions, by=c(UNIPROTKB,SGD,ORF,GNAME,PNAME)) %>%
            #left_join(df.functions, by=c(UNIPROTKB,SGD,ORF,GNAME,PNAME)) %>%
            #left_join(df.phenotypes, by=c(UNIPROTKB,SGD,ORF,GNAME,PNAME))
...
We also obtain **`r TOTAL_FEATURES`** protein features.
