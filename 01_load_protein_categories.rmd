...
df.aa_mw = DUB %>% 
       group_by(UNIPROT) %>% 
       summarise( pepstats.mean_MW = MW/prot.size, L = prot.size ) %>%
       left_join(df.protinfo, by=c('UNIPROT' = 'UNIPROTKB')) %>%
       filter(qual != 'Dubious' & EXISTENCE != "Uncertain" & L>50  )

TRANSCRIPTOMICS$pepstats.AA_costly = df.aa_mw %>% dplyr::filter(pepstats.mean_MW > 119 ) %>% pull(UNIPROT)
...
fullsti = tibble( uni=AA.COUNT$id, uniprot.sti_full = AACOUNT2SCORE(COUNT=AA.COUNT[,1:20],SCORE=get.stickiness()) )
df.aascales = DUB %>%
