...
# Turn orthologs abundance as columns *use integrated ppm values*
PPM.ORTHO.4932 = PPM.ORTHO %>% ungroup() %>% filter(taxid==4932) %>% 
                  ...
                  pivot_wider(id_cols=c(NOG,orf,ppm_4932,organ), 
                              names_from=taxid, names_glue="ppm_{taxid}",
                              values_from=ppm_int, values_fn=mean_) %>%
                  dplyr::select(-ppm_NA,-organ)
...
### Amino-acid biosynthetic cost
df.uni_info = UNIPROT %>%  dplyr::select(c(UNIPROTKB,REVIEWED,EXISTENCE,SCORE,qual))
df.aaMW = DUB %>% 
       group_by(UNIPROT) %>% 
       summarise( pepstats.mean_MW = MW/prot.size ) %>%
       left_join(df.uni_info, by=c('UNIPROT' = 'UNIPROTKB'))

head(df.aaMW)

df.aaMW %>% dplyr::filter(pepstats.mean_MW > 120 ) 


TRANSCRIPTOMICS$pepstats.AA_costly = df.aaMW %>% dplyr::filter(pepstats.mean_MW > 120 ) %>% pull(UNIPROT)
TRANSCRIPTOMICS$pepstats.AA_cheap = df.aaMW %>% dplyr::filter(pepstats.mean_MW < 105 ) %>% pull(UNIPROT)
...
df.orthoexp =  PPM.ORTHO.4932 %>% 
                 group_by(NOG,orf) %>%
                 mutate( ortho_ppm_avg = mean_(c_across(starts_with('ppm_'))),
                         ortho_ppm_sd  = sd_(c_across(starts_with('ppm_'))),
                         ortho_ppm_cv  = ortho_ppm_sd/ortho_ppm_avg,
                         ortho_ppm_max  = max_(c_across(starts_with('ppm_'))),
                         ortho_ppm_min  = min_(c_across(starts_with('ppm_'))),
                         ortho_ppm_n= sum.na(c_across(starts_with('ppm_')),notNA = T) ) %>%
                  filter(ortho_ppm_n > 1)
...
df.aa = AA.FR %>% mutate(uni=names(UNI.SC)) %>% relocate(uni) %>%
        rename_with(.fn = Pxx, px="uniprot.f",s='_',.cols=-uni)
...
INTERACTIONS$intact.megahub_ppi = cent.INTACT %>% dplyr::filter(cent_deg>50) %>% pull(ids)
INTERACTIONS$intact.superhub_ppi = cent.INTACT %>% dplyr::filter(between(cent_deg,20,50)) %>% pull(ids)
INTERACTIONS$intact.minihub_ppi = cent.INTACT %>% dplyr::filter(between(cent_deg,10,20)) %>% pull(ids)
...
saveRDS(PROT,sprintf("output/proteome-properties-%s.rds",when))
