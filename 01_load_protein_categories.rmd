...
### Gene duplication ----------------------------------------------------------
df.wgd  = OHNO %>% dplyr::select(orf,orf_dup=dup.orf,RLEN,PID1,pid,score_B100 = SCORE.B100,S,N,G) %>% dplyr::rename_with(.fn=Pxx, px="byrne2005")
...
df.aaMW = DUB %>% 
       group_by(UNIPROT) %>% 
       summarise( pepstats.mean_MW = MW/prot.size ) 

TRANSCRIPTOMICS$pepstats.costly = df.aaMW %>% dplyr::filter(pepstats.mean_MW > 120 ) %>% pull(UNIPROT)
TRANSCRIPTOMICS$pepstats.cheap = df.aaMW %>% dplyr::filter(pepstats.mean_MW < 105 ) %>% pull(UNIPROT)
...
names(superfamilies) = paste0("superfamilies.",make_clean_names(names(superfamilies),case='none'))
...
names(pfam_clans) = paste0("pfamclan.",make_clean_names(names(pfam_clans),case='none'))
...
df.biophysics = PROTEOME %>% 
                left_join(df.aa,by=c('UNIPROTKB'='uni')) %>%
...
names(CYC_complexes) = paste0("CYC.",janitor::make_clean_names(names(CYC_complexes),case='none'))
...
names(EBI_complexes) = paste0("EBI.",janitor::make_clean_names(names(EBI_complexes),case='none'))
...
names(kegg_path) = paste0("keggpath.", make_clean_names(names(kegg_path), case='none') )
kegg_module = keep(split(MODULE$id,MODULE$desc),function(X){ length(X) > 9  })
names(kegg_module) = paste0("keggmod.",make_clean_names(names(kegg_module), case='none') )
...
names(signatures) = paste0("lee2014.",make_clean_names(names(signatures), case='none') )

PHENOTYPES=append(PHENOTYPES,signatures)

#### Subcellular localization 
PHENOTYPES$uniloc.foci = UNILOC %>% filter(HAS_FOCI) %>% pull(id)
PHENOTYPES$uniloc.isoform  = UNILOC %>% filter(HAS_ISOFORM) %>% pull(id)
loc = split(uniloc$id,uniloc$loc) 
subloc = keep(loc,function(X){ length(X)>30 })
names(subloc) = paste0("uniloc.", make_clean_names(names(subloc),case='none',))
...
for( C in names(PROPERTIES)){
  PROT[,C] = (PROT$UNIPROTKB %in% PROPERTIES[[C]]) | (PROT$ORF %in% PROPERTIES[[C]]) | (PROT$GNAME %in% PROPERTIES[[C]])
}
head(PROT)
...
when='07_04_2021'
PROT=readRDS(sprintf("data/proteome-properties-%s.rds",when))
FEATURES=readRDS(sprintf("data/proteome-features-%s.rds",when))
