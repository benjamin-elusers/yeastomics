...
cent.STRING = network.centrality(STRING[,c('ORF1','ORF2')] %>% distinct(), namenet='STRING')
...
INTERACTIONS$intact.megahub_ppi = cent.INTACT %>% dplyr::filter(cent_deg>20) %>% pull(ids)
INTERACTIONS$intact.superhub_ppi = cent.INTACT %>% dplyr::filter(between(cent_deg,11,20)) %>% pull(ids)
INTERACTIONS$intact.minihub_ppi = cent.INTACT %>% dplyr::filter(between(cent_deg,5,10)) %>% pull(ids)
